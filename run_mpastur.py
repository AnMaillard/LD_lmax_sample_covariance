...
#The run file for the MC runs with nu = mpastur
...
if __name__== "__main__":
    ...
    nu = "mpastur"
    gamma = 1. #The MP scaling ratio
    parameters = {'gamma':gamma}
    global_seed = int(time.time())
    np.random.seed(global_seed)
    NB_DIAGONALS = 5 #Nb of new independent runs
    verbosity = 2
    n = 500
    NB_CYCLES = 100
    ...
    LENGTH_CYCLE = 5000
    alpha = 2.
    t_list = np.linspace(0.18,0.,num = 10)
    t_list = t_list[:2]
    partial_seeds = np.random.randint(1e6, size = len(t_list))
    power_method = True #Do we use power methods or eigh
